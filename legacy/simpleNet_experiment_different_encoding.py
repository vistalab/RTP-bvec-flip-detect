...
from preprocess import dataLoader, flipAxes, getTensorList, dataLoaderEncodingCuda
...
class simpleNet(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(12*12*12*3, 200)
...
def testIt():
    ...
    allWrongs = []
    allWrongNames = []
    for batch_idx, (data, target, batchNames) in enumerate(dataLoaderEncodingCuda(test, batchSize, randomAxis=True, shuffle=False, names=names, resizeFactor=None)):
        data_temp = np.copy(data)
        data, target = Variable(data), Variable(target)
        data, target = data.cuda(), target.cuda()
        data = data.view(-1, 12*12*12*3)
...
tensors, names = pickle.load(open("encodeList12.p", "rb"))
...
for epoch in range(epochs):
    epochAcc = []
    for batch_idx, (data, target, batchNames) in enumerate(dataLoaderEncodingCuda(train, batchSize, randomAxis=True, shuffle=True, names=names, maxShift=None, resizeFactor=None)):
        data, target = Variable(data), Variable(target)
...
for epoch in range(epochs):
    epochAcc = []
    for batch_idx, (data, target, batchNames) in enumerate(dataLoaderEncodingCuda(train, batchSize, randomAxis=True, shuffle=False, names=names, maxShift=None)):
        data, target = Variable(data), Variable(target)
        data, target = data, target.cuda()
        data = data.view(-1, 12*12*12*3)
        # data = data.permute(0,4,1,2,3)
...
torch.save(net.state_dict(), "trained_simplenet_encoded.torch")
