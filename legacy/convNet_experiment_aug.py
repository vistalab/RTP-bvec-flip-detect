...
tensors, names = pickle.load(open("tensorsAndNamesFull.p", "rb"))
...
for epoch in range(epochs):
    epochAcc = []
    for batch_idx, (data, target, batchNames) in enumerate(dataLoaderCuda(train, batchSize, randomAxis=True, shuffle=True, names=names, maxShift=6)):
        data, target = Variable(data), Variable(target)
        data, target = data, target.cuda()
        #data = data.view(-1, 24*24*24*6)
        data = data.permute(0,4,1,2,3)
...
for epoch in range(epochs):
    epochAcc = []
    for batch_idx, (data, target, batchNames) in enumerate(dataLoaderCuda(train, batchSize, randomAxis=True, shuffle=False, names=names, maxShift=6)):
        data, target = Variable(data), Variable(target)
        data, target = data, target.cuda()
        #data = data.view(-1, 24*24*24*6)
        data = data.permute(0,4,1,2,3)
