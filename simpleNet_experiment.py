...
from src.features.build_features import flipAxes
...
def test():
    ...
    allWrongs = []
    allWrongNames = []
    for batch_idx, (data, target, batchNames) in enumerate(dataLoader(testData, batchSize, names=testNames, simulationFunction=flipAxes, randomAxis=True, shuffle=False)):
...
def train():
    for epoch in range(epochs):
        epochAcc = []
        lossArr = []
        for batch_idx, (data, target, batchNames) in enumerate(dataLoader(trainData, batchSize, names=trainNames, simulationFunction=flipAxes, randomAxis=True, shuffle=True)):
            data, target = Variable(data), Variable(target)
            data, target = data.cuda(), target.cuda()
            data = data.view(-1, dimIn)
            optimizer.zero_grad()
...
test_interval = 10
batchSize = 10
tensorDir = "data/processed/tensorsAndNames48to12.p"
...
tensors, names = pickle.load(open("data/processed/tensorsAndNames48to12.p", "rb"))
